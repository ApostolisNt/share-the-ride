...
const RidesCard = ({ ride }: any) => {
  ...
  const rideExpired = date < formatDate(currentDate);

  return (
    <>
      {rideExpired && (
        <div className="ride_card_expired absolute left-1/2 top-1/2 rounded-md border-2 border-solid border-red-600 p-2 font-semibold uppercase text-red-600 opacity-80">
          This ride has expire
        </div>
      )}
      <div
        className={`ride_card shadow-card transition-shadow-transform duration-200 ease-out hover:-translate-y-2 hover:shadow-cardHover ${rideExpired && "pointer-events-none opacity-50"}`}
